...
\hfill \emph{solution} \refAnswer{2d1}
...
Two points on a sphere are antipodal if the line connecting them passes through the center of the sphere. They are 'directly opposite' one another, or maximally distant from one another while still being on the sphere. 
	...
	\end{enumerate}
\hfill \emph{solution} \refAnswer{2d3}
\end{Exercise}
...
\section{Logarithms}
\begin{Exercise}[title={A Better Antiderivative}, difficulty=0, label=7a2]
We defined the function $L(x):=\displaystyle \int_1^x \dfrac{1}{t}dt$.
	\begin{enumerate}
		\item This definition combined with FTC part 2 gives us what relationship between $L(x)$ and the function $f(x)=\dfrac{1}{x}$.\\
		\item Calculate the derivative of $g(x)=\ln|x|$.\\
		\item Explain why part a and part b do not contradict the theorem that tells us if a function $f$ has antiderivatives $F$ and $G$ that $F$ and $G$ must differ by a constant.\\
...
\begin{Exercise}[title={}, difficulty=0, label=7a3]
...
Show that for any $0<x<y$, we have the inequality:\\[.5cm] $\dfrac{1}{y} < \dfrac{\ln(y)-\ln(x)}{y-x}< \dfrac{1}{x}$.
 \hfill \emph{solution} \refAnswer{7a1}
...
\begin{Exercise}[title={$x^y$ vs $y^x$}, difficulty=2, label=7c1]
Let $f(x)=\dfrac{\ln(x)}{x}$
\begin{enumerate}
	\item Determine on what intervals $f$ is strictly increasing versus strictly decreasing.\\
	\item Use part (a) to show that if $e \leq A < B$ then $A^B > B^A$.\\
	\item Use part (b) to show that $e^\pi > \pi^e$.\\
\end{enumerate}
\hfill \emph{solution} \refAnswer{7c1}
\end{Exercise}

\begin{Answer}[ref={7c1}]
After finding f' you can show that f is decreasing on the interval $(e,\infty)$. This implies that since $e<\pi$ we get $f(e)>f(\pi)$. Then multiplying through by $e*\pi$ and taking exponentials base $e$ of both sides gives the result.
\end{Answer}
